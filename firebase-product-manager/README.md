...
### Simulated write denied (Rules Playground)
Jika di Rules Playground muncul **"Simulated write denied (viewing outdated simulation)"**, biasanya karena:
1. Anda menekan **Run** pada simulator lama sebelum rules terbaru dipublish.
2. Simulasi `set/update` tidak menyertakan `userId` yang sama dengan `auth.uid`.
3. Simulasi tidak mengisi context Auth (UID kosong), sehingga `auth != null` gagal.

Contoh data simulasi yang benar untuk create:
...
Tips:
- Klik **Publish** rules dulu, lalu buka ulang Rules Playground.
- Isi Auth UID simulator dengan UID user yang valid.
- Gunakan path sesuai node (`products`, `categories`, `suppliers`).
